...
function updateAnchors() {
  ...
  anchor1 = createVector(
    origin.x + 200 * cos(-angleRad), 
    origin.y + 200 * sin(-angleRad)
  );
  // Точка на полу (расстояние 200 от угла)
  anchor2 = createVector(origin.x + 200, origin.y); 
}
...
class Ball {
  ...
  getAcc(p) {
    ...
    return p5.Vector.add(f1, f2).add(gravity).div(params.mass);
  }
...
function drawEnvironment() {
  ...
  // Стена
  let angleRad = radians(params.mu);
  line(origin.x, origin.y, origin.x + 600 * cos(-angleRad), origin.y + 600 * sin(-angleRad));
  
  // Заштрихованная зона угла
  noStroke();
  fill(200, 200, 200, 100);
  arc(origin.x, origin.y, 80, 80, -angleRad, 0);
}
...
function createUI() {
  ...
  sliders.l1 = createSlider(50, 300, params.l1, 1).position(x, y + 160);
  sliders.l2 = createSlider(50, 300, params.l2, 1).position(x, y + 200);
  
  sel = createSelect().position(x, y + 250).style('width', '140px');
  ...
  resetButton = createButton('Reset Position').position(x, y + 290);
...
function drawLabels() {
  ...
  text(`K1 (наклон): ${params.k1}`, x, 115);
  text(`K2 (пол): ${params.k2}`, x, 155);
  text(`L1: ${params.l1}`, x, 195);
  text(`L2: ${params.l2}`, x, 235);
  
  fill(0);
  text(`МЕТОД: ${params.method}`, 20, 350);
}